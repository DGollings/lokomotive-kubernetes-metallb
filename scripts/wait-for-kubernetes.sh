...
while ! $KUBECTL get nodes 2>/dev/null | grep -v NotReady; do
  # Be quiet when output is pointless
  if $KUBECTL get nodes 2>/dev/null | grep 'no such host'; then
    sleep 20
  # but start announcing yourself when nodes are getting ready
  elif $KUBECTL get nodes 2>/dev/null | grep NotReady >/dev/null; then
